...
void* MemoryMappedFile::create(const std::string& filename, unsigned long long len, bool zero) {
    ...
    void* p = map(cc().getOperationContext(), filename.c_str(), len);
    if (p && zero) {
...
void* MemoryMappedFile::map(OperationContext* txn, const char* filename) {
    ...
    return map(txn, filename, l);
}
void* MemoryMappedFile::mapWithOptions(OperationContext* txn, const char* filename, int options) {
    unsigned long long l;
    try {
        l = boost::filesystem::file_size(filename);
    } catch (boost::filesystem::filesystem_error& e) {
    ...
    return map(txn, filename, l, options);
}
